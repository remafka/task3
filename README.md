...
2. Приложение отправляет запрос на Сервердля получения меню выбранного ресторана.
...
11. Приложение отображает Пользователюстатус заказа ("Подтвержден" или "Отклонен").
...
"Возврат средств" (если заказ отменен и требуется возврат) 
 ...
 Конкретные статусы и переходы будут зависеть от требований к приложению YumExpress.
...
Это упрощенная диаграмма классов. В реальной системе YumExpress будет значительно больше классов и связей между ними. Детализация диаграммы зависит от конкретных требований к системе.
.
